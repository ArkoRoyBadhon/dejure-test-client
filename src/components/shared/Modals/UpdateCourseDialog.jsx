...
export default function UpdateCourseDialog({ course, open, onOpenChange }) {
  ...
  useEffect(() => {
    if (course) {
      const milestones = course.milestones?.map((m, index) => {
        return {
          title: m.title || "",
      ...
      setFormData({
        ...
        customSlug: course.customSlug || "",
        thumbnailType: course.thumbnailType || "image",
        description: course.description || "",
        category: course.category?._id || "",
        subCategory: course.subCategory || "",
        types: course.types || [{ mode: "online", price: "", salePrice: "" }],
        durationMonths: course.durationMonths || "",
        batchNo: course.batchNo || "",
        seat: course.seat || "",
        startFrom: course.startFrom?.split("T")[0] || "",
        payTerm: course.payTerm || ["one-time"],
        milestones,
        ...
        tags: course.tags || "",
        totalAdmittedStudents: course.totalAdmittedStudents || "",
        totalLiveClasses: course.totalLiveClasses || "",
      });

      if (course.thumbnail && course.thumbnailType === "image") {
        setThumbnailPreview(
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.thumbnailType === "image") {
      ...
      } else if (thumbnailPreview && thumbnailPreview.includes("http")) {
        // Keep existing image
      } else {
        submission.append("image", "");
      }
    ...
    try {
      await updateCourse({ id: course._id, patch: submission }).unwrap();
      ...
      if (err?.data?.message) {
        ...
        toast.error("❌ You don't have permission to update courses.");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} className="">
      <DialogContent className="custom-dialog-width max-h-[90vh] overflow-y-auto">
        <DialogTitle className="text-3xl font-semibold text-gray-800 mb-8 text-center">
          Update Course
        </DialogTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        onChange={(e) =>
                          ...
                          setFormData((prev) => ({
                            ...
                            types: prev.types.map((t, i) =>
                              i === index ? { ...t, mode: e.target.value } : t
                            ),
                          }))
                        }
                        className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Uncheck to deactivate this course
              </p>
            </div>
