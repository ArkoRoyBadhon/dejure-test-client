...
function normalizeCategories(categories) {
  return (categories || []).map((cat, idx) => ({
    ...
    questions: (cat.questions || []).map((q) =>
      typeof q === "string" ? q : q._id || q.id
    ),
    customQuestions: cat.customQuestions || [],
  }));
}

export const QuestionSetDetailsUpdate = ({
  formData,
  setFormData,
  onBack,
  onSubmit,
  ...
  // Normalized categories for local state
  const [categories, setCategories] = useState(
    normalizeCategories(formData.categories)
  );
  ...
  const [currentCustomQuestion, setCurrentCustomQuestion] = useState({
    ...
    correctAnswer: "", // String for correct option text
    subQuestions: [],
    mark: 1,
  });
  ...
  useEffect(() => {
    const normalized = normalizeCategories(formData.categories);
  ...
  const handleAddCustomQuestion = () => {
    ...
    setCurrentCustomQuestion({
      ...
      type: formData.type,
      options: [], // Array of strings
      correctAnswer: "", // String for correct option
      subQuestions: [],
      mark: 1,
    });
  ...
  const handleSaveCustomQuestion = () => {
    ...
    let updatedCustomQuestion = { ...currentCustomQuestion };
    if (
      currentCustomQuestion.type === "WRITTEN" &&
      (!currentCustomQuestion.subQuestions ||
      ...
      updatedCustomQuestion = {
        ...
        subQuestions: [
          {
            id: Date.now().toString(),
            subQuestionText: updatedCustomQuestion.questionText,
            mark: updatedCustomQuestion.mark,
          },
        ],
      };
    }
      ...
      prev.map((cat) => {
        if (cat.id === activeCategory) {
          ...
          if (editingCustomQuestionIndex >= 0) {
            ...
            updatedCustomQuestions.push(updatedCustomQuestion);
          }
  ...
  const handleEditCustomQuestion = (cq, idx) => {
    setCurrentCustomQuestion({ ...cq });
    setEditingCustomQuestionIndex(idx);
    setIsCustomQuestionDialogOpen(true);
  };
    ...
    setCategories((prev) =>
      prev.map((cat) =>
        cat.id === activeCategory
          ? {
              ...cat,
              customQuestions: cat.customQuestions.filter((cq, i) => i !== idx),
            }
          : cat
      )
    );
              ...
              <div className="md:col-span-2">
                ...
                {isLoading ? (
                  ...
                  <div className="p-4 border rounded-lg bg-red-50 text-red-600">
                    {fetchError?.message || "No questions available right now"}
                  </div>
                ...
                {/* Custom Questions below existing questions */}
                {activeCategory && (
                  <div className="mt-4">
                        ...
                        >
                            ...
                            {cq.type === "WRITTEN" &&
                              cq.subQuestions?.length > 0 && (
                                <ul className="ml-4 list-disc">
                                  {cq.subQuestions.map((sq, i) => (
                                    <li key={sq.id}>
                                      {sq.subQuestionText} ({sq.mark} marks)
                                    </li>
                                  ))}
                                </ul>
                              )}
                            {cq.type === "MCQ" && cq.options?.length > 0 && (
                              <ul className="ml-4 list-disc">
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              size="sm"
                              variant="ghost"
                              onClick={() => handleEditCustomQuestion(cq, idx)}
                            >
                              <Pencil className="w-4 h-4" />
                            </Button>
                            <Button
